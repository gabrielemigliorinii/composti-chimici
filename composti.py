...
class Composto:
    ...
    def __init__(self, formula):
        ...
        self.massa_molare = self.__mm()

    # CH4 => [[C,1],[H,4]]
    # H2O => [[H,2],[O,1]]
    # C2H7NO2 => [['C', 2], ['H', 7], ['N', 1], ['O', 2]]
    def __split(self):

        for f in self.formula:
            if f not in self.__a:
                return -1
        ...
        for i in range(len(self.formula)):
            ...
            if self.formula[i:i+2] in self.__elements and len(self.formula[i:i+2]) > 1:
                if (self.formula[i+2].isnumeric()):
                    obj += [self.formula[i:i+2],(int)(self.formula[i+2])] 
            else:
                if self.formula[i:i+1] not in self.__elements:
                    return -1
                try:
                    if (self.formula[i+1].isnumeric()):
                        obj += [self.formula[i:i+1],(int)(self.formula[i+1])]
                    else:
...
def Main():
    ...
    while 1:
        formula = input("\n\n - Inserire formula: ")
        if formula in ("-1", "0", "exit"):
            os.system("CLS")
            exit()
        try:
            composto = Composto(formula)
            print(" \n - Massa molare di "+formula+": "+ str(composto.massa_molare[0]) + " [" + composto.massa_molare[1]+"]")
        except:
            print(" \n - General Error")
        print("\n\n")
        os.system("PAUSE")
